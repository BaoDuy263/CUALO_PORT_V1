...
export class ContainerCreateComponent implements OnInit {
  ...
  get type() {
    return this.CreateEditForm.get('type');
  }
  get side() {
    return this.CreateEditForm.get('side');
  }
  get dateCheckOut() {
    return this.CreateEditForm.get('dateCheckOut');
  }
  get activity() {
    return this.CreateEditForm.get('activity');
  }
  get location() {
    return this.CreateEditForm.get('location');
  }
  get state() {
    return this.CreateEditForm.get('state');
  }
  get region() {
    return this.CreateEditForm.get('region');
  }
  ...
  checkShowBtn() {
    ...
    this.isSave = false;
    if (
      (this.CreateEditForm.value.step == 3 &&
        this.CreateEditForm.value.activity == 5) ||
      (this.CreateEditForm.value.step == 5 &&
        this.CreateEditForm.value.activity == 2) ||
      (this.CreateEditForm.value.step == 4 &&
        this.CreateEditForm.value.activity == 2) ||
      (this.CreateEditForm.value.step == 2 &&
        this.CreateEditForm.value.activity == 5)
    )
      this.isSave = true;
    this.isSave = false;
      ...
      if (
        !(
          (
            ...
            this.CreateEditForm.value.activity == 9 // Đóng hàng
          ) // Rút ruột
        )
      ) {
  ...
  saveTrans() {
    ...
    if (this.CreateEditForm.value.step == 2) {
      ...
      if (
        ...
        //Kiểm tra status Rút ruột
        if (!(this.CreateEditForm.value.status == 1)) {
          // trạng thái F => status==1
          alert('Sai phương án, vui lòng kiểm tra lại !');
          return;
        }
      }
    ...
    if (
      ...
      (this.CreateEditForm.value.weight == 2.3 ||
        this.CreateEditForm.value.weight == 4)
    ) {
    ...
    dialogRef.componentInstance.title =
      'Bạn có chắc chắn muốn thay đổi trạng thái hiện tại không?';
    dialogRef.componentInstance.button = 'Đóng';
    dialogRef.componentInstance.buttonConfirm = 'Xác nhận';
    this.CreateEditForm.value.activity = parseInt(
      this.CreateEditForm.value.activity
    );
    this.CreateEditForm.value.typeDelivery = parseInt(
      this.CreateEditForm.value.typeDelivery
    );
  ...
  getDetailTrans(id: number) {
    this.transactionService.GetDetailTrans(id).subscribe((res) => {
      return res;
    });
  }
  ...
  handleSelect(value: any) {
    ...
    const index = this.lstVehicle.findIndex(
      (item: any) => item.licensePlates === value
    );
    if (index < 0) {
      return;
    }
  ...
  handleSelectUser(value: any) {
    ...
    const index = this.lstUserDeport3.findIndex(
  ...
  printE() {
    this.transactionService.GetDetailTrans(this.transId).subscribe((res) => {
      if (res == null) {
        const dialogRef = this.dialog.open(ContainerPopupComponent);
        dialogRef.componentInstance.title =
          'Vui lòng lưu giao dịch trước khi in!';
  ...
  changeActivity(e: any) {
    this.checkShowBtn();
   // alert(this.currentActivity +'#####'+ this.CreateEditForm.value.activity);

    // Reset biển số xe nếu phương án hiện tại là trả rỗng sang cấp rỗng
    //alert(this.CreateEditForm.value.licensePlates);
    ...
    for (let i = 0; i < lstCheckTD.length; i++) {
      ...
      if (checkCurrActivity) {
        ...
        this.openPopup(
          lstCheckTD[i].alert,
          lstCheckTD[i].newStep,
          lstCheckTD[i]?.newStepCancel
        );
  ...
  removeInfomation() {
    var mType =  this.CreateEditForm.value.type ;
    this.CreateEditForm.patchValue({
      commodity: null,
      seal1: null,
      customer: null,
      ...
      lstVehicle: null,
      weight:
        mType!=null? (this.CreateEditForm.value.type.substring(0, 2) == '20' ? 2.3 : 4.0):0,
      //btt
    });
  }
