...
export class PerformCreateComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.loadVehicles();
    this.transactionService.GetDetailTrans(this.performId).subscribe(res => {
      this.CreateEditForm = new FormGroup({
  ...
  onSubmit() {
    ...
    if (this.CreateEditForm.valid && this.isCreate === false) {
      this.transactionService.UpdateTrans(this.performId, this.CreateEditForm.value).subscribe(response => {
